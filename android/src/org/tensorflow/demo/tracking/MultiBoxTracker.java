...
public class MultiBoxTracker {
  ...
  // [HHJ]
  private static final boolean DISPLAY_CONFIDENCE = false;
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if (DISPLAY_CONFIDENCE) {
        final String labelString = String.format("conf: %.2f", recognition.detectionConfidence);
        final Rect textRect = new Rect();
        borderedText.getTextBounds(labelString, 0, labelString.length(), textRect);
        final float x = trackedPos.left;
        final float y = Math.min(trackedPos.bottom + textRect.height() + 10, canvas.getHeight());
        borderedText.drawText(canvas, x, y, labelString);
      }
    }
  }
